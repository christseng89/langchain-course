...
def demo_interrupt_for_approval():
  ...
  feedback_text = "Make it more concise and add specific mention of the company culture"
  ...
  print("\n   Graph path taken:")
  print("   START -> [draft] -> ⏸️ PAUSE -> human feedback -> [approval] -> [finalize] -> END")
...
def demo_iterative_review():
  ...
  graph.add_conditional_edges("submit", route_after_review, {"apply": "apply", "done": "done"})
  ...
  print("                                       └── else -> [apply] -> ⏸️ [submit] (LOOP)")
  ...
  app.update_state(config, {"review_comments": [feedback_2], "status": "needs_revision"})
